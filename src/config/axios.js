...
axiosInstance.interceptors.request.use(
  (config) => {
    // Attach any available tokens. Use separate headers for admin and user tokens.
    const adminToken = localStorage.getItem('adminToken');
    const userToken = localStorage.getItem('userToken') || localStorage.getItem('token');
    if (adminToken) config.headers[env.VITE_ADMIN_TOKEN_NAME] = adminToken;
    if (userToken) config.headers[env.VITE_USER_TOKEN_NAME] = userToken;

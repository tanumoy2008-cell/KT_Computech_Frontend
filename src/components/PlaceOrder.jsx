...
const PlaceOrder = () => {
  ...
  const isCancellable = (order) => {
    ...
    const forbidden = ['Cancelled', 'Completed', 'Delivered', 'Shipped'];
  ...
  return (
    <>
            ...
            <div key={order._id} className="border rounded-lg p-4 bg-white shadow-sm">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  {order.customer?.name && <div className="text-sm text-gray-700">{order.customer.name}</div>}
                </div>
                <div className="text-right">
                  <div className="font-semibold">{formatCurrency(order.total)}</div>
                  <div className={`text-sm mt-1 ${order.status === 'Completed' ? 'text-green-600' : 'text-amber-600'}`}>{order.status}</div>
                </div>
              </div>
              ...
              <div className="mt-3 border-t pt-3">
                {order.items?.slice(0,3).map((it, idx) => (
                  <div key={idx} className="flex justify-between text-sm py-1">
                    <div className="truncate max-w-[300px]">{it.name} × {it.qty}</div>
                    <div>{formatCurrency(it.total)}</div>
                  </div>
                ))}
                {order.items?.length > 3 && (
                  <div className="text-xs text-gray-500 mt-1">+{order.items.length - 3} more items</div>
                )}
              </div>
      ...
      <OrderModal order={selectedOrder} onClose={() => setSelectedOrder(null)} onCancel={handleCancel} isCancellable={isCancellable(selectedOrder)} onReorder={handleReorder} />
    </>
  );
};
...
const OrderModal = ({ order, onClose, onCancel, isCancellable, onReorder }) => {
  ...
  return (
    <AnimatePresence>
      <motion.div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
        <motion.div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <div className="space-y-2 text-sm">
                <p className="text-xs text-gray-500">You can cancel this order within 1 hour from the time it was placed. After 1 hour cancellation will not be allowed. You may also Re-order items from this order if it is completed or cancelled.</p>
                <div className="h-2" />
                <p><span className="font-medium">Date:</span> {formatDate(order.createdAt)}</p>
                <p><span className="font-medium">Status:</span> <span className="ml-1 px-2 py-1 rounded-full text-xs bg-gray-100 text-gray-800">{order.status}</span></p>
                <p className="flex items-center"><span className="font-medium mr-2">Payment Method:</span> {getPaymentMethodIcon(order.paymentMode)} {order.paymentMode}</p>
              </div>

              <h3 className="font-semibold text-lg mt-6 mb-2">Customer Details</h3>
              <div className="space-y-2 text-sm">
                {order.customer?.name && (<p><span className="font-medium">Name:</span> {order.customer.name}</p>)}
                {order.customer?.email && (<p><span className="font-medium">Email:</span> {order.customer.email}</p>)}
                {order.customer?.phone && (<p><span className="font-medium">Phone:</span> {order.customer.phone}</p>)}
                {order.customer?.address && (<p className="mt-2"><span className="font-medium block">Address:</span>{order.customer.address}</p>)}
              </div>
            </div>
            ...
            <div>
                ...
                {order.items?.map((item, idx) => (
                  <div key={idx} className="p-3 border-b last:border-b-0 flex justify-between items-center">
                    ...
                    <div className="text-right">
                      ...
                      {item.off > 0 && (<p className="text-xs text-green-600">{item.off}% off</p>)}
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-4 space-y-2 text-sm">
                ...
                <div className="flex justify-between font-semibold text-lg pt-2 border-t mt-2"><span>Total:</span><span>{formatCurrency(order.total)}</span></div>
              </div>
            </div>
          </div>

          <div className="mt-6 pt-4 border-t flex justify-end space-x-3">
            {onReorder && (order.status === 'Completed' || order.status === 'Cancelled') && (
              <button

  ...
  return (
    <div id='productSection' className='w-full min-h-screen px-10 py-10'>
      <div className='w-full flex flex-col gap-y-2'>
        <h1 className='text-center font-ArvoBold text-5xl'>Categories</h1>
        ...
        {categories.map((items,i)=>(
          <Link to={items.link} key={i} className='flex flex-col gap-y-2 items-center cursor-pointer'>
            <div className='w-30 h-30 lg:w-40 lg:h-40 xl:w-40 xl:h-40 2xl:w-40 2xl:h-40 rounded-full overflow-hidden border-black border-4'>
              <img className='w-full h-full object-cover brightness-80' src={items.pic} alt="" />
            </div>
            <h1 className='lg:text-lg xl:text-lg 2xl:text-2xl font-semibold font-Geist'>{items.name}</h1>
          </Link>
        ))}
        </div>
      </div>
        <h1 className='text-center mb-5 text-4xl font-ArvoBold'>Our Top Product's</h1>
        <div className='grid md:grid-cols-2 md:grid-rows-4 lg:grid-cols-3 lg:grid-rows-3 xl:grid-cols-3 xl:grid-rows-3 2xl:grid-cols-4 2xl:grid-rows-2 justify-around content-center gap-5'>

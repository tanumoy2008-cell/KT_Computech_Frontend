...
const Billing = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      productData.forEach(product => addProductToList(product));
      setSearchValue('');
    } catch (err) {
  ...
  const updateColorVariant = (productId, colorVariantId) => {
    setProducts(products.map(p => p._id === productId ? { ...p, selectedColorVariantId: colorVariantId } : p));
  };
            ...
            if (paymentMode === 'UPI' && res.data?.qrCode) {
              ...
              if (action === true || action === 'true') {
                try {
                  ...
                  await Swal.fire({ title: 'Copy UPI URI', input: 'text', inputValue: upiUri, showCancelButton: true });
                }
              }
            } else {
    ...
    <div className='h-screen w-full bg-zinc-800 px-10 py-5 flex flex-col gap-y-5'>
      ...
      <div className="flex gap-x-2 items-center">
        <select
          ...
          className="px-3 py-2 rounded-md bg-white outline-none border border-zinc-300"
        >
        ...
        <input
          ...
          className="px-3 py-2 rounded-md flex-1 bg-white outline-none border border-zinc-300"
        />

        <button
          onClick={handleSearch}
          className="bg-green-600 text-white font-PublicSans py-2 px-3 rounded text-xl cursor-pointer transition-colors duration-200 hover:bg-green-700"
        >
          Search
        </button>
      </div>
          ...
          <tbody>
            {products.map((p) => (
              <tr key={p._id} className="text-center border-t">
                ...
                <td className="px-4 py-2">
                  ₹{(Number(p.price || 0) * (1 - (Number(p.off || 0) / 100))).toFixed(2)}
                </td>
                <td className="px-4 py-2">
                  <input
                    type="number"
                ...
                <td className="px-4 py-2">
                  {((Number(p.price || 0) * (1 - (Number(p.off || 0) / 100))) * p.quantity).toFixed(2)}
                </td>
                <td className="px-4 py-2">
                  <button onClick={() => deleteProduct(p.barcode)} className="bg-red-500 text-white px-2 py-1 rounded">
                    Delete
                  </button>
                </td>
              </tr>
      ...
      <div className="flex flex-col gap-y-3 mt-4 bg-white px-4 py-4 rounded-lg">
        ...
        <div className="flex justify-between items-center">
          ...
          <button
            onClick={createOrder}
            className='bg-green-600 px-20 py-2 rounded text-white text-xl font-PublicSans font-semibold'
          >
            Pay
          </button>
        </div>

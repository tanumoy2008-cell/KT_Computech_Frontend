  ...
  const displayPrice = off > 0
    ...
    >
      <Link to={`/product-dets/${data._id}`} onClick={() => clickHandel(data._id)} className="block h-full">
        {/* Badge */}
            {off > 0 && (
              <span className={`${variant === 'list' ? 'absolute' : 'absolute top-4 right-4'} z-10 bg-red-500 text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg transform rotate-6`}>
                {off}% OFF
              </span>
            )}
        
        {/* Image Container */}
        <div className={`relative overflow-hidden bg-gray-100 ${variant === 'compact' ? 'h-40' : variant === 'list' ? 'w-28 h-28 flex-shrink-0' : 'h-64'} border-b-1 border-zinc-500/70`}>
          <motion.img
            initial={{ scale: 1 }}
            whileHover={{ scale: 1.05 }}
            transition={{ duration: 0.5 }}
            className={`${variant === 'compact' ? 'w-full h-full object-cover rounded' : 'w-full h-full object-cover transition-transform duration-500'}`}
            src={data.productPic}
            alt={data.name}
            loading="lazy"
          />
        ...
        {/* Product Info */}
  <div className={`${variant === 'compact' ? 'p-3' : variant === 'list' ? 'flex-1 p-0 pl-4' : 'p-4'}`}>
          {/* Category & Company */}
          <div className={`flex ${variant === 'list' ? 'justify-start items-center gap-3 mb-1' : 'justify-between items-center mb-1'}`}>
            <span className={`text-xs text-gray-700 font-bold uppercase tracking-wider ${variant === 'compact' ? 'text-[11px]' : ''}`}>
              {data.Subcategory || 'Category'}
            </span>
            <span className={`text-sm font-semibold text-green-600 uppercase bg-green-50 px-2 py-1 rounded ${variant === 'compact' ? 'text-xs px-1 py-0.5' : ''}`}>
              {data.company}
            </span>
          </div>
          
          {/* Product Name */}
          <h3 className={`font-semibold text-gray-800 mb-2 line-clamp-2 ${variant === 'compact' ? 'text-sm h-10' : variant === 'list' ? 'text-base' : 'h-12 flex items-center'}`}>
            {data.name}
          </h3>
          
          {/* Price & CTA */}
          <div className={`${variant === 'list' ? 'flex items-center justify-between mt-0' : 'flex justify-between items-center mt-3 pt-3 border-t-2 border-gray-400'}`}>
            <div>
            ...
            >
              ...
              <span className={`${variant === 'compact' ? 'text-sm' : ''}`}>Add to Cart</span>
            </motion.button>
          </div>
        </div>
      </Link>

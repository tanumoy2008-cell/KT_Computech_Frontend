...
const ProductAdder = () => {
  ...
  .filter(Boolean)
  .forEach(desc => {
    formData.append("productDescription", desc.trim());
  })
    if (data.avatar) formData.append("avatar", data.avatar);
    try {
      toast.info("please wait we Upload the product.")
  ...
  return (
    <div className='h-screen w-full bg-zinc-800 px-10 py-5 flex flex-col gap-y-5'>
            <div className='w-full py-10 px-10 bg-white rounded-2xl text-5xl flex items-center justify-between'>
                <h1 className='font-Syne uppercase font-semibold'>Total Products</h1>
                <p className='font-ZenMeduim font-semibold'>{totalProduct}</p>
            </div>
            ...
            />
                ...
                <div className='h-full w-[40%]'>
                    ...
                    <form onSubmit={handleSubmit(formSubmit)} className='flex flex-col gap-y-2'>
                        ...
                        placeholder='Product Description. . .(eg. #des1 #des2)' className='w-full outline-none border-1 py-2 px-4 h-50 rounded-lg resize-none'></textarea>
                        </div>
                        ...
                        type='reset' className='w-full text-center py-3 outline-none rounded-lg text-white font-Jura uppercase text-2xl transition-colors duration-200 bg-red-400 active:bg-red-600'>Reset</button>
                        </div>
                    </form>
                </div>
                </div>
